...
const AddRoom = () => {
    ...
    return (
        <form>
            <Title align='left' font='outfit' title='Add Room' subTitle='Provide complete and accurate room information, pricing, and amenities to ensure a smooth and reliable booking experience for users.' />
            ...
            <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
                {Object.keys(images).map((key)=>(
                    <label htmlFor={`roomImages${key}`} key={key}>
                        ...
                        <input type="file" accept='images/*' id={`roomImages${key}`} hidden onChange={e=>setImages({...images, [key]: e.target.files[0]})} />
                    </label>
                ))}
            </div>
